 ...
 * real navigations. Gated on CUA_E2E_LIVE=1 + KERNEL_API_KEY so it never
	...
	it(
		...
		async () => {
			await withLiveBrowser(async (cdpWsUrl) => {
				...
				const snapshot = textOf(await first.execute({ type: "browser_snapshot", filter: "interactive" } as CuaBrowserAction));
				...
				const scoped = textOf(await second.execute({ type: "browser_snapshot", ref } as CuaBrowserAction));
				...
				await expect(legacy.execute({ type: "browser_click", ref } as CuaBrowserAction)).rejects.toThrow(/stale/);
				legacy.close();
				...
				await expect(third.execute({ type: "browser_click", ref } as CuaBrowserAction)).rejects.toThrow(/stale/);
				third.close();
			});
		},
		120_000,
