...
export async function waitForBrowserExpectation(runtime: BrowserWaitRuntime, options: BrowserWaitOptions): Promise<BrowserWaitForResult> {
	...
	try {
		...
		baseline = options.baseline ?? await beforeDeadline(() => runtime.observeTarget(targetId), remaining(), now);
		if (expired()) return timedOut(started, now());
	} catch (error) {
	...
	while (!expired()) {
		...
		try { observation = await beforeDeadline(() => runtime.observeTarget(targetId), remaining(), now); }
		catch (error) { if (error instanceof WaitDeadlineError) break; return failedObservation(started, now(), error, initial, final); }
...
async function beforeDeadline<T>(start: () => Promise<T>, remaining: number, now: () => number): Promise<T> {
	if (remaining <= 0) throw new WaitDeadlineError();
	const began = now();
	let timer: ReturnType<typeof setTimeout> | undefined;
	try {
		const result = await (Number.isFinite(remaining)
			? Promise.race([start(), new Promise<never>((_resolve, reject) => { timer = setTimeout(() => reject(new WaitDeadlineError()), remaining); })])
			: start());
		if (Number.isFinite(remaining) && now() - began >= remaining) throw new WaitDeadlineError();
		return result;
	} catch (error) {
		if (error instanceof WaitDeadlineError || Number.isFinite(remaining) && now() - began >= remaining) throw new WaitDeadlineError();
		throw error;
	} finally {
