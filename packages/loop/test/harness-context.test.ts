...
describe("harness tool context", () => {
	it("delivers the exact supplied context object to custom harness tools", async () => {
		...
		const cwd = mkdtempSync(join(tmpdir(), "loop-harness-context-"));
	...
	it("runs pi's native read/write/edit/bash tools against the context's execution env", async () => {
		const cwd = mkdtempSync(join(tmpdir(), "loop-harness-tools-"));
		const harness = await openHarness<ExecutionToolContext>({
			models: modelsFromStream(scriptedStream([
				(model) => assistant(model, [{ type: "toolCall", id: "write-1", name: "write", arguments: { path: "notes.txt", content: "hello loop\n" } }], "toolUse"),
		...
		expect(results.get("read")?.content.some((block) => block.text?.includes("goodbye loop"))).toBe(true);
		expect(results.get("bash")?.content.some((block) => block.text?.includes("goodbye loop"))).toBe(true);
	});
});
