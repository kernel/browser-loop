...
import { createLoopModels, getLoopModel, OPENAI_COMPUTER_USE_API } from "../src/pi/index";
...
// pi's builtin transport iterates an event stream; the Loop native-computer
...
describe("OpenAI adapter routing", () => {
	it("streams a plain function-tool context through pi's builtin transport", async () => {
		...
		const message = await createLoopModels().streamSimple(model, {
		...
		// field on function_call items, unlike the Loop adapter.
	...
	it("reaches the Loop adapter when the model carries OPENAI_COMPUTER_USE_API", async () => {
		responsesCreate.mockReturnValueOnce({
			id: "resp_2",
			output: [{
				type: "computer_call",
		...
		// compileLoopToolCatalog derives this api onto the model whenever OpenAI's
		// native computer tool is selected; the provider wrapper dispatches on it
		// alone, with no request-shape sniffing.
		const computerModel = { ...model, api: OPENAI_COMPUTER_USE_API };
		const message = await createLoopModels().streamSimple(computerModel, {
			...
			apiKey: "test",
			loopIncomingToolPlan: { openaiComputerName: "computer", googleNames: {}, googleExcludedNames: [], nativeToolNames: ["computer"] },
		} as never).result();

		// Only the Loop native-computer adapter understands computer_call items;
	...
	it("reaches the Loop adapter when the transcript carries a deferred tool-search addition", async () => {
		responsesCreate.mockReturnValueOnce({
			id: "resp_3",
			status: "completed",
			usage: { input_tokens: 1, output_tokens: 1 },
		...
		// The Loop adapter round-trips the namespace pi's builtin drops.
		const call = message.content.find((part): part is ToolCall => part.type === "toolCall") as (ToolCall & { namespace?: string }) | undefined;
	...
	it("keeps cache-relevant payload fields identical across a mid-conversation escalation", async () => {
		// A turn can move from pi's builtin transport to the Loop adapter mid-session
		...
		await createLoopModels().streamSimple(model, {
			messages: [{ role: "user", content: "look it up", timestamp: 1 }],
			tools,
