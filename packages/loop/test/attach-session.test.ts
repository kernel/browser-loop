...
async function openSession(options: {
	...
	tools: readonly LoopHarnessTool<undefined>[];
	models?: Models;
}): Promise<{
	harness: AgentHarness;
	session: Session;
	select: (model: LoopModelInput, tools: readonly LoopHarnessTool<undefined>[]) => Promise<void>;
}> {
...
describe("compiling a pair", () => {
	it("compiles exact native-only, native-plus-Loop, Playwright-only, and browser-act-only catalogs", () => {
		const custom = callerTool("customer_lookup");
		const cases = [
			{
				model: "anthropic:claude-opus-5" as const,
				tools: [loop.providers.anthropic.tools.browser(), custom],
				names: ["browser", "customer_lookup"],
			},
			{
				model: "openai:gpt-5.5" as const,
				tools: [loop.providers.openai.tools.computer(), loop.tools.browser.snapshot(), loop.tools.browser.act()],
				names: ["computer", "browser_snapshot", "browser_act"],
			},
			{ model: "openai:gpt-5.5" as const, tools: [loop.tools.playwright()], names: ["playwright_execute"] },
			{ model: "openai:gpt-5.5" as const, tools: [loop.tools.browser.act()], names: ["browser_act"] },
		];
	...
	it("resolves a ref through the supplied collection, falling back to the registry", () => {
		...
		const first = { ...getLoopModel("openai:gpt-5.5"), baseUrl: "https://first.example" };
		const second = { ...getLoopModel("openai:gpt-5.6-sol"), baseUrl: "https://second.example" };
	...
	it("refuses a native tool the model cannot take, before anything is applied", async () => {
		...
		await expect(select("openai:gpt-5.5", [loop.providers.anthropic.tools.browser()])).rejects.toThrow(/requires a anthropic model/);
	...
	it("retains protocol-required OpenAI native computer screenshot results outside the image replay limit", async () => {
		...
		const model = getLoopModel("openai:gpt-5.5");
		...
		const compiled = handle.compile({ model, tools: [loop.providers.openai.tools.computer(), callerTool("ordinary")] });
...
describe("applying a pair to a live harness", () => {
	it("streams the transport the selection derives", async () => {
		...
		const { harness, select } = await openSession({
			...
			tools: [loop.tools.browser.snapshot()],
		...
		await select("google:gemini-3.6-flash", loop.providers.google.toolsets.browser());
		expect(harness.getModel().api).toBe(GOOGLE_INTERACTIONS_API);

		await harness.prompt("go");
		expect(streamedApis).toEqual([GOOGLE_INTERACTIONS_API]);
	});
	...
	it("records one model change for a switch that moves both the model and its transport", async () => {
		...
		await select("openai:gpt-5.5", [loop.tools.browser.snapshot()]);
	...
	it("ignores a release from a pair that is no longer live", async () => {
		...
		const session = await new InMemorySessionRepo().create();
		const handle = attach({ browser, client, models });
		const first = handle.compile({ model: "google:gemini-3.6-flash", tools: [loop.tools.browser.snapshot()] });
		const harness = new AgentHarness({
			session,
			model: first.model,
			models: first.models,
		...
		await handle.compile({ model: "google:gemini-3.6-flash", tools: loop.providers.google.toolsets.browser() }).apply(harness);
		...
		releaseFirst();
		await handle.compile({ model: "google:gemini-3.6-flash", tools: [loop.tools.browser.snapshot()] }).apply(harness);
	...
	it("clears failed-turn state before the next prompt", async () => {
		...
		const { harness } = await openSession({
			...
			tools: [loop.providers.anthropic.tools.browser(), failing, succeeding],
