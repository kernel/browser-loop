...
import * as google from "../src/pi/providers/google/provider";

// This adapter is exercised directly (not through compileLoopToolCatalog's
// derivation), so the model must carry the Interactions api itself.
const model: Model<typeof google.GOOGLE_INTERACTIONS_API> = { ...getLoopModel("google:gemini-3.6-flash"), api: google.GOOGLE_INTERACTIONS_API };
const incoming = {
...
describe("Google Interactions computer-use adapter", () => {
	...
	it("fails with a named catalog error when Google emits an excluded predefined function", async () => {
		...
		}, { apiKey: "test", loopIncomingToolPlan: screenshotIncoming }).result();
