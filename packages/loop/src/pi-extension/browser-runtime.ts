 ...
 * deleted.
 *
 * This holds `LoopExecutionResources` rather than a full `attach()` handle: pi
 ...
 */
export class LoopBrowserRuntime {
	private pending?: Promise<LoopExecutionResources>;
	private resources?: LoopExecutionResources;
	...
	async get(signal?: AbortSignal): Promise<LoopExecutionResources> {
		if (signal?.aborted) throw new Error("Loop browser provisioning cancelled");
		if (this.closed) throw new Error("Loop browser runtime is closed");
	...
	private async provision(): Promise<LoopExecutionResources> {
		const apiKey = this.env.KERNEL_API_KEY;
		if (!apiKey) throw new Error("KERNEL_API_KEY is required when a Loop tool first executes");
	...
	async close(): Promise<void> {
		...
		let pendingResources: LoopExecutionResources | undefined;
		try {
