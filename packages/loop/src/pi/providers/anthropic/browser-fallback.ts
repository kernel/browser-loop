...
import type { LoopAnthropicBrowserFallback } from "../../../core/tool-catalog";
...
function streamWithFallback(
	...
	const fallback = options?.loopIncomingToolPlan?.anthropicBrowserFallback;
...
async function detectAccessAndRelay(
	...
	model: Model<Api>,
	context: Context,
	options: LoopSimpleStreamOptions,
	fallback: LoopAnthropicBrowserFallback,
	credential: string,
): Promise<void> {
	let committed = false;
...
function replaceNativeDeclaration(payload: unknown, fallback: LoopAnthropicBrowserFallback): unknown {
...
export function isAnthropicBrowserAccessError(
	...
	fallback: Pick<LoopAnthropicBrowserFallback, "beta" | "nativeType">,
): boolean {
