...
export const GOOGLE_CUA_INTERACTIONS_API = "google-cua-interactions";
...
export const streamGoogleInteractions: StreamFunction<typeof GOOGLE_CUA_INTERACTIONS_API, GoogleInteractionsOptions> = (
	model,
	context,
	options,
) => {
...
export const streamSimpleGoogleInteractions: StreamFunction<typeof GOOGLE_CUA_INTERACTIONS_API, GoogleInteractionsOptions> =
...
async function run(
	...
	try {
		...
		const threading = responseThreadingEnabled(options)
			? responseThreadingDelta(context.messages, GOOGLE_CUA_INTERACTIONS_API)
		...
		const nativeToolNames = new Set(options?.cuaIncomingToolPlan?.nativeToolNames ?? []);
		const ordinaryToolNames = new Set((context.tools ?? []).map((tool) => tool.name).filter((name) => !nativeToolNames.has(name)));
		for (const raw of contents) emitContent(stream, output, raw, options?.cuaIncomingToolPlan, ordinaryToolNames);
...
function emitContent(
	...
	raw: unknown,
	incoming: CuaIncomingToolPlan | undefined,
