import type { BrowserActionAct, BrowserActionSnapshot, BrowserActStep, BrowserExpectation } from "../actions/index";
...
export interface BrowserActRuntime {
	...
	/** Stop starting input sub-operations when aborted and settle any in-flight atomic input before resolving. */
	executeStep(step: BrowserActStep, tabId: string | undefined, signal: AbortSignal): Promise<void>;
	wait(expect: BrowserExpectation, baseline: BrowserObservation, targetId: string, tabId?: string, timeoutMs?: number, pollMs?: number): Promise<BrowserWaitForResult>;
	evaluate(expect: BrowserExpectation, observation: BrowserObservation, baseline: BrowserObservation): BrowserExpectationEvaluation;
	present(observation: BrowserObservation, action: BrowserActionSnapshot): BrowserPresentation;
...
export async function runBrowserAct(action: BrowserActionAct, runtime: BrowserActRuntime): Promise<BrowserActResult> {
	...
	for (let index = 0; index < action.steps.length; index += 1) {
		...
		try {
			await executeBeforeDeadline((signal) => runtime.executeStep(step, action.tab_id, signal), deadline);
		...
		if (!timeout && !undispatched) {
			try {
				if (step.expect) {
					...
					if (waitResult.status === "timed_out" && remaining(deadline) === 0) timeout = deadline.reason;
				}
				if (!timeout) {
	...
	if (action.expect && (!stopReason || terminalNavigation)) {
		try {
			...
			const globalTimeout = result.status === "timed_out" && remaining(globalDeadline) === 0;
			timedOut ||= globalTimeout;
			if (globalTimeout) { stopReason = "global_timeout"; stoppedAt = finalStepIndex; }
	...
	for (let attempt = 0; attempt < 3 && !successor; attempt += 1) {
		try {
			...
			const complete: BrowserActionSnapshot = { type: "browser_snapshot", tab_id: action.tab_id, depth: Number.MAX_SAFE_INTEGER };
...
/** Abort a step at its deadline, then wait for its in-flight atomic input to settle before returning. */
async function executeBeforeDeadline<T>(operation: (signal: AbortSignal) => Promise<T>, deadline: ActDeadline): Promise<T> {
	const timeoutMs = remaining(deadline);
	if (timeoutMs <= 0) throw new BrowserActDeadlineError(deadline.reason);
	const controller = new AbortController();
	let expired = false;
	const timer = setTimeout(() => {
		expired = true;
		controller.abort(new BrowserActDeadlineError(deadline.reason));
	}, timeoutMs);
	try {
		const value = await operation(controller.signal);
		if (expired || remaining(deadline) === 0) throw new BrowserActDeadlineError(deadline.reason);
		return value;
	} catch (error) {
		if (expired || remaining(deadline) === 0) throw new BrowserActDeadlineError(deadline.reason);
		throw error;
...
function stepResult(index: number, step: BrowserActStep, outcome: BrowserActStepResult["outcome"], diagnostics: string[], expectation?: BrowserActExpectationEvidence): BrowserActStepResult {
