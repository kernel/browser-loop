...
export class BrowserExecutor {
	...
	async execute(action: BrowserAction, signal?: AbortSignal): Promise<BatchReadResult[]> {
		throwIfAborted(signal);
	...
	private async dispatch(action: BrowserAction, signal?: AbortSignal): Promise<BatchReadResult[]> {
		switch (action.type) {
	...
	private async snapshot(action: BrowserActionSnapshot): Promise<string> {
	...
	private waitFor(action: BrowserActionWaitFor): Promise<BrowserWaitForResult> {
		return this.waitForExpectation(action.expect, { timeoutMs: action.timeout_ms, pollMs: action.poll_ms, tabId: action.tab_id });
	}

	private waitForExpectation(expect: BrowserExpectation, options: { timeoutMs?: number; pollMs?: number; tabId?: string; baseline?: BrowserObservation; targetId?: string }): Promise<BrowserWaitForResult> {
	...
	private act(action: BrowserActionAct): Promise<BrowserActResult> {
	...
	private async executeActStep(step: BrowserActStep, tabId: string | undefined, signal: AbortSignal): Promise<void> {
		throwIfAborted(signal);
		switch (step.type) {
	...
	private evaluateRefExpectation(
		expectation: Extract<BrowserExpectation, { type: "ref" }>,
	...
	private presentObservation(observation: BrowserObservation, action: BrowserActionSnapshot): BrowserPresentation {
	...
	private async find(action: BrowserActionFind): Promise<string> {
	...
	private async click(action: BrowserActionClick, signal?: AbortSignal): Promise<void> {
		throwIfAborted(signal);
	...
	private async hover(action: BrowserActionHover, signal?: AbortSignal): Promise<void> {
		throwIfAborted(signal);
		const targetId = await this.resolveTarget(action.tab_id);
		throwIfAborted(signal);
		const session = await this.attach(targetId);
	...
	private async fill(action: BrowserActionFill, signal?: AbortSignal): Promise<void> {
		throwIfAborted(signal);
		const targetId = await this.resolveTarget(action.tab_id);
	...
	private async scrollTo(action: BrowserActionScrollTo, signal?: AbortSignal): Promise<void> {
		throwIfAborted(signal);
		const targetId = await this.resolveTarget(action.tab_id);
	...
	private async scroll(action: BrowserActionScroll): Promise<void> {
		const session = await this.session(tabOf(action));
	...
	private async key(action: BrowserActionKey, signal?: AbortSignal): Promise<void> {
		throwIfAborted(signal);
		const session = await this.session(tabOf(action));
	...
	private async navigate(action: BrowserActionNavigate, signal?: AbortSignal): Promise<string> {
		throwIfAborted(signal);
		const targetId = await this.resolveTarget(action.tab_id);
		const session = await this.attach(targetId);
		throwIfAborted(signal);
	...
	private async resolvePoint(
		action: BrowserActionClick | BrowserActionHover,
		targetId: string,
		session: string,
...
const CURSOR_SCAN_GROUP = "loop-cursor-scan";
