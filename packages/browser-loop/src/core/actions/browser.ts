...
export type BrowserExpectation = BrowserExpectationLeaf | { all: NonEmptyArray<BrowserExpectationLeaf> } | { any: NonEmptyArray<BrowserExpectationLeaf> };

export interface BrowserActionWaitFor {
...
type BrowserActStepOptions = {
	/** Timeout for performing this step and verifying its expectation, capped by the plan deadline. An in-flight atomic input settles before timeout is reported. */
	timeout_ms?: number;
	expect?: BrowserExpectation;
};
...
export type BrowserActStep = (
	...
	| { type: "wait"; ms?: number }
) & BrowserActStepOptions;

/** Dependent action plan whose optional `expect` verifies the complete plan result. */
export interface BrowserActionAct {
	type: "browser_act";
	steps: NonEmptyArray<BrowserActStep>;
	expect?: BrowserExpectation;
	/** Global timeout for performing every step and verifying the plan expectation. An in-flight atomic input settles before timeout is reported. */
	timeout_ms?: number;
	poll_ms?: number;
...
export interface BrowserActionText {
	type: "browser_text";
	tab_id?: string;
}

export interface BrowserActionFind {
	type: "browser_find";
	query: string;
	tab_id?: string;
}
...
export interface BrowserActionFill {
	type: "browser_fill";
	ref: string;
...
export interface BrowserActionScrollTo {
...
export interface BrowserActionTypeText {
	type: "browser_type";
	text: string;
	tab_id?: string;
}

export interface BrowserActionKey {
	type: "browser_key";
	text: string;
	repeat?: number;
	tab_id?: string;
...
export interface BrowserActionListTabs {
...
export interface BrowserActionEvaluate {
...
export interface BrowserActionSchemaOptions {
	/**
...
export function createBrowserActionSchemaByType(options: BrowserActionSchemaOptions): Record<BrowserActionType, TSchema> {
	...
	const actStepOptions = {
		timeout_ms: Type.Optional(Type.Number({ minimum: 1, maximum: 30_000, description: "Timeout for performing this step and verifying its expectation, capped by the plan deadline. An in-flight atomic input settles before timeout is reported." })),
		...
		browser_act: Type.Object(
			{
				...
				expect: Type.Optional(expectation),
				timeout_ms: Type.Optional(Type.Number({ minimum: 1, maximum: 30_000, description: "Global timeout for performing all steps and verifying the plan expectation (default 30000). An in-flight atomic input settles before timeout is reported." })),
