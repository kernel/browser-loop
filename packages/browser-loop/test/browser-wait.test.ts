...
import type { BrowserExpectation } from "../src/index";
import {
	type AXNode,
...
import {
	type BrowserRefResolver,
	evaluateBrowserExpectation,
	waitForBrowserExpectation,
} from "../src/core/translator/browser-wait";
...
describe("evaluateBrowserExpectation", () => {
	...
	] as Array<[BrowserExpectation, boolean]>) ("evaluates %#", (condition, truth) => {
	...
	] as Array<[boolean, undefined, BrowserExpectation, boolean | undefined]>) ("combines three-valued condition %#", (_a, _b, condition, truth) => {
...
describe("waitForBrowserExpectation", () => {
	...
	it("retains baseline evidence when observation completes at the deadline", async () => {
		let time = 0;
		const result = await waitForBrowserExpectation({
			selectTarget: async () => "page",
			observeTarget: async () => {
