...
import { getLoopModel, type LoopSimpleStreamOptions } from "../src/pi/index";
import { compileLoopToolCatalog, loop } from "../src/index";
import { withAnthropicBrowserFallback } from "../src/pi/providers/anthropic/browser-fallback";
...
describe("Anthropic native browser access fallback", () => {
	it("retries with the equivalent function tool and remembers the credential", async () => {
		...
		const catalog = compileLoopToolCatalog({
			model,
			requestedTools: [loop.providers.anthropic.tools.browser()],
		});
		...
		const options: LoopSimpleStreamOptions = {
